            ...
            <el-col :span="16">
                <div class="timezone-switcher">
                    <el-button 
                        size="small" 
                        :icon="ArrowLeft" 
                        @click="previousTimezone"
                    />
                    <div class="timezone-display">{{ currentTimezoneName }}</div>
                    <el-button 
                        size="small" 
                        :icon="ArrowRight" 
                        @click="nextTimezone"
...
import { Clock, CopyDocument, AlarmClock, ArrowLeft, ArrowRight } from '@element-plus/icons-vue'
...
// 时区列表
const timezones = [
    { label: 'CST/+8', value: 'Asia/Shanghai' },
    { label: 'UTC/+0', value: 'UTC' },
    { label: 'EST/-5', value: 'America/New_York' },
    { label: 'PST/-8', value: 'America/Los_Angeles' },
    { label: 'GMT/+0', value: 'Europe/London' },
    { label: 'JST/+9', value: 'Asia/Tokyo' },
]

// 当前时区名称
const currentTimezoneName = computed(() => {
    return timezones.find(tz => tz.value === timezone.value)?.label || timezone.value
})

// 切换到上一个时区
const previousTimezone = () => {
    const currentIndex = timezones.findIndex(tz => tz.value === timezone.value)
    const newIndex = currentIndex <= 0 ? timezones.length - 1 : currentIndex - 1
    timezone.value = timezones[newIndex].value
}

// 切换到下一个时区
const nextTimezone = () => {
    const currentIndex = timezones.findIndex(tz => tz.value === timezone.value)
    const newIndex = currentIndex >= timezones.length - 1 ? 0 : currentIndex + 1
    timezone.value = timezones[newIndex].value
}
